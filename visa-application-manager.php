...
function wpforms_update_entry()
{
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $entryId = intval($_POST['entry_id']);
        $updatedData = isset($_POST['data']) ? $_POST['data'] : []; // Access data directly from POST

        // Log entry ID and updated data for debugging
        ...
        if ($updated !== false) {
            ...
            error_log('Failed to update entry: ' . $wpdb->last_error);
...
add_action('wp_ajax_update_wpforms_entry', 'wpforms_update_entry');
add_action('wp_ajax_nopriv_update_wpforms_entry', 'wpforms_update_entry');
...
function create_user_info_table()
{
    ...
    // create table for visa fee detils

    $visa_fee_table = $wpdb->prefix . 'visa_fees';
    $sql12 = "CREATE TABLE $visa_fee_table (
        ...
        service_available varchar(100) NOT NULL,
        visa_fee varchar(100) NOT NULL,
        PRIMARY KEY  (id)
    ) $charset_collate;";
    dbDelta($sql12);
...
// Fix for SVG rendering in the editor
function fix_svg()
{
    echo '<style type="text/css">
        .attachment-svg { background: none; }
    </style>';
}
